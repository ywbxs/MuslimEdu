...
function ReportDetailModal({
  ...
  // Reset the catch-up form each time a different (or no) month is opened,
  // so a previous month's draft doesn't leak into the next one.
  useEffect(() => {
    setNote('');
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmitMonth(month.key, { note, academic_rating: academicRating, wellbeing_rating: wellbeingRating }, photos);
    } finally {
      setIsSubmitting(false);
    }
  };

  const showCatchUpForm = canCatchUp && !report;
            ...
            <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View style={styles.modalEmptyWrap}>
                ...
                <Text style={styles.modalEmptyBody}>
                  You can still submit a late report for {month?.name}.
                </Text>
              </View>

              <SectionHead
                icon={<IconEdit color={EMERALD} />}
                title="How was that month?"
                subtitle="Tell us about your studies, activities, and how you were feeling."
              />
              <View style={styles.noteWrap}>
                <TextInput
                  style={styles.noteInput}
                  placeholder="Write a short note about that month..."
              ...
              <SectionHead
                ...
                subtitle="Rate your academic performance that month."
              />
              <RatingSelector value={academicRating} onChange={setAcademicRating} labels={{ 1: 'Poor', 3: 'Average', 5: 'Excellent' }} />

              <SectionHead
                icon={<IconHeart color={EMERALD} />}
                title="Wellbeing Rating"
                subtitle="Rate your overall wellbeing that month."
              />
              <RatingSelector value={wellbeingRating} onChange={setWellbeingRating} labels={{ 1: 'Very Low', 3: 'Average', 5: 'Very High' }} />
              ...
              <View style={styles.thumbRow}>
                {Array.from({ length: MAX_PHOTOS }).map((_, i) => {
                  ...
                  if (photo) {
                    return (
                      <TouchableOpacity key={photo.uri} style={styles.thumb} onPress={() => removePhoto(photo.uri)} activeOpacity={0.8}>
                  ...
                  return (
                    <TouchableOpacity key={`slot-${i}`} style={styles.thumbEmpty} onPress={pickPhotos} activeOpacity={0.7}>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    <Text style={styles.submitButtonText}>Submit Late Report</Text>
                  </>
                )}
              </TouchableOpacity>
            </ScrollView>
...
export default function OrphanReportScreen() {
  ...
  // Lets a child catch up on a missed past month, the same way an admin can
  // already back-date a report for any child via report_month.
  const handleSubmitForMonth = async (
    monthKey: string,
    fields: { note: string; academic_rating: number; wellbeing_rating: number },
    monthPhotos: PickedPhoto[],
  ) => {
    if (!token) return;
    try {
      await submitReport(token, { ...fields, report_month: monthKey }, monthPhotos);
      Alert.alert('Report submitted', 'Your late report has been sent to your school admin.');
      await load();
  ...
  return (
    <View style={styles.flex}>
      ...
      <ReportDetailModal
        ...
        canCatchUp={!!selectedMonth && selectedMonth.key <= currentMonthKey}
        onSubmitMonth={handleSubmitForMonth}
      />
    </View>
  );
}
