...
/**
 * A cashier's full profile, admin-facing. Mirrors AdminTeacherProfileScreen:
 * tapping the pencil switches the info cards to editable TextInputs. Save
 * calls admin_accountant_profile_update with name/email/phone/address/
 * gender/birthday and an optional password reset (only sent when non-empty,
 * so leaving it blank keeps the current password).
 */
export default function CashierProfileScreen() {
  const insets = useSafeAreaInsets();
  const navigation = useNavigation();
  const route = useRoute();
  const { cashierId, cashierName } = (route.params as { cashierId: number; cashierName: string }) ?? {};
  const { token } = useAuth();
  const { t } = useLocale();
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchCashierProfile(token, cashierId);
      setProfile(data);
      setFields({
        name: data.name ?? '',
        email: data.email ?? '',
      ...
      setError(err instanceof Error ? err.message : t('cashier_profile.load_error', 'Failed to load profile.'));
    }
  }, [token, cashierId, t]);
  ...
  const handleSave = async () => {
    ...
    if (!fields.name.trim() || !fields.email.trim()) {
      Alert.alert(t('cashier_profile.missing_info_title', 'Missing info'), t('cashier_profile.missing_info_message', 'Name and email are required.'));
      return;
    }
    setIsSaving(true);
    try {
      const updateFields: CashierBasicProfileFields = {
      ...
      await updateCashierProfile(token, cashierId, updateFields);
      setFields((p) => ({ ...p, password: '' }));
      setIsEditing(false);
      Alert.alert(t('cashier_profile.saved_title', 'Saved'), t('cashier_profile.saved_message', "The cashier's profile has been updated."));
      load();
    } catch (err) {
      Alert.alert(t('cashier_profile.save_error', 'Could not save'), err instanceof Error ? err.message : t('common.try_again_full', 'Please try again.'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <View style={styles.flex}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>{cashierName ?? t('cashier_profile.title', 'Profile')}</Text>
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          {isEditing ? (
            <>
              <EditField label={t('cashier_profile.name', 'Name')} value={fields.name} onChangeText={(v) => setFields((p) => ({ ...p, name: v }))} />
              <EditField label={t('cashier_profile.email', 'Email')} value={fields.email} onChangeText={(v) => setFields((p) => ({ ...p, email: v }))} keyboardType="email-address" autoCapitalize="none" />
              <EditField label={t('cashier_profile.phone', 'Phone')} value={fields.phone} onChangeText={(v) => setFields((p) => ({ ...p, phone: v }))} keyboardType="phone-pad" />
              <EditField label={t('cashier_profile.address', 'Address')} value={fields.address} onChangeText={(v) => setFields((p) => ({ ...p, address: v }))} />
              <EditField label={t('cashier_profile.gender', 'Gender')} value={fields.gender} onChangeText={(v) => setFields((p) => ({ ...p, gender: v }))} placeholder={t('cashier_profile.gender_placeholder', 'male / female')} autoCapitalize="none" />
              <EditField label={t('cashier_profile.birthday', 'Birthday')} value={fields.birthday} onChangeText={(v) => setFields((p) => ({ ...p, birthday: v }))} placeholder="YYYY-MM-DD" />
              <EditField label={t('cashier_profile.new_password', 'New Password')} value={fields.password} onChangeText={(v) => setFields((p) => ({ ...p, password: v }))} placeholder={t('cashier_profile.password_placeholder', 'Leave blank to keep current password')} secureTextEntry autoCapitalize="none" />
              ...
              <View style={styles.editActionsRow}>
                ...
                <TouchableOpacity style={[styles.editActionBtn, styles.editActionBtnPrimary]} onPress={handleSave} disabled={isSaving}>
                  {isSaving ? <ActivityIndicator color="#FFFFFF" size="small" /> : <Text style={styles.editActionBtnPrimaryText}>{t('cashier_profile.save', 'Save')}</Text>}
                </TouchableOpacity>
              </View>
            </>
          ) : (
            <>
              <View style={styles.infoCard}>
                <InfoRow label={t('cashier_profile.email', 'Email')} value={profile.email} />
              </View>
              <View style={styles.infoCard}>
                <InfoRow label={t('cashier_profile.phone', 'Phone')} value={profile.phone} />
              </View>
              <View style={styles.infoCard}>
                <InfoRow label={t('cashier_profile.address', 'Address')} value={profile.address} />
              </View>
              <View style={styles.infoCard}>
                <InfoRow label={t('cashier_profile.gender', 'Gender')} value={profile.gender} />
              </View>
              <View style={styles.infoCard}>
                <InfoRow label={t('cashier_profile.birthday', 'Birthday')} value={profile.birthday} />
              </View>
              <View style={styles.infoCard}>
                <InfoRow label={t('cashier_profile.staff_code', 'Staff Code')} value={profile.code} />
              </View>
            </>
          )}
        </ScrollView>
