...
import { buildTablePdf, PdfColumn } from '../../utils/pdfExport';
...
export default function AcademicAnalyticsScreen() {
  ...
  const load = useCallback(async () => {
    if (!token) return;
    try {
      const [a, b] = await Promise.all([fetchAcademicAnalytics(token), fetchAttendanceTrend(token)]);
      setData(a);
      setTrend(b);
    } catch {
  ...
  const onExportPdf = async () => {
    ...
    try {
      const columns: PdfColumn[] = [
        { label: t('academic_analytics.pdf_col_metric', 'Metric'), width: 260 },
        { label: t('academic_analytics.pdf_col_value', 'Value'), width: 200 },
      ];
      const rows: string[][] = [
        [t('academic_analytics.school_snapshot', 'SCHOOL SNAPSHOT'), ''],
        [t('academic_analytics.students', 'Students'), String(data.summary.students)],
        [t('academic_analytics.teachers', 'Teachers'), String(data.summary.teachers)],
        [t('academic_analytics.attendance', 'Attendance'), data.summary.attendance_rate == null ? '—' : `${data.summary.attendance_rate}%`],
        [t('academic_analytics.grade_average', 'Grade average'), data.summary.grade_average == null ? '—' : String(data.summary.grade_average)],
        ['', ''],
        [t('academic_analytics.attendance_by_month', 'ATTENDANCE BY MONTH'), ''],
        ...trend.map((x) => [x.month, `${x.present + x.late}/${x.present + x.late + x.absent + x.excused} present`]),
        ['', ''],
        [t('academic_analytics.enrollment_statuses', 'ENROLLMENT STATUSES'), ''],
        [t('academic_analytics.active_enrollments', 'Active enrollments'), String(data.enrollment.active)],
        ...Object.entries(data.enrollment.statuses).map(([k, v]) => [k, String(v)]),
        ['', ''],
        [t('academic_analytics.generated', 'Generated'), new Date(data.generated_at).toLocaleString()],
      ];
      const pdf = buildTablePdf(t('academic_analytics.title', 'Academic Analytics'), columns, rows);
