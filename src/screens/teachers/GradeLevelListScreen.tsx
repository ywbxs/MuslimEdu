...
const GradeLevelListScreen = () => {
  ...
  const fetchGradeLevels = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to load grade levels');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleDelete = (gradeLevel) => {
    Alert.alert(
      'Delete Grade Level',
      `Delete "${gradeLevel.name}"? This can't be undone.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const msg = error.response?.data?.message || 'Failed to delete grade level';
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        <TextInput
          ...
          placeholder="Search grade level name or code..."
          value={searchTerm}
          ...
          <EmptyState
            ...
            subtitle={
              searchTerm
                ...
                : `No ${statusFilter === 'all' ? '' : statusFilter + ' '}grade levels yet.`
            }
            colors={theme}
          />
        }
