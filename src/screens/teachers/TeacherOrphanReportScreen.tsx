...
export default function TeacherOrphanReportScreen() {
  ...
  const timelineMeta: TimelineMonthMeta[] = timeline.map((m) => ({
    key: m.key,
    name: m.name,
    submitted: m.submitted,
    meters: [
      { label: t('teacher_orphan_report.rating_teaching', 'Teaching'), value: m.report?.teaching_effectiveness_rating ?? null },
      { label: t('teacher_orphan_report.rating_engagement', 'Engagement'), value: m.report?.classroom_engagement_rating ?? null },
      { label: t('teacher_orphan_report.rating_growth', 'Growth'), value: m.report?.professional_growth_rating ?? null, tone: 'gold' },
    ],
    photos: m.report?.photos ?? [],
  }));
  const pressTimelineMonth = (meta: TimelineMonthMeta) => {
    const original = timeline.find((x) => x.key === meta.key);
    if (!original) return;
